...
class ModelService {
  ...
  static Future<bool> testAssetAccess() async {
    try {
      ...
      final modelFiles = [
        'assets/models/Llama-3.2-1B-Instruct-q4f16_0-MLC 6.36.51 PM.zip',  // LLaMA 3.2 1B MLC model
      ];
  ...
  static Future<List<AIModel>> getAvailableModels() async {
    ...
    try {
      ...
      if (await dir.exists()) {
        ...
        // Use LLaMA 3.2 1B MLC model
        final modelFiles = [
          'Llama-3.2-1B-Instruct-q4f16_0-MLC 6.36.51 PM.zip',    // LLaMA 3.2 1B MLC model
        ];
      ...
      if (models.isEmpty) {
        ...
        for (final modelFileName in ['Llama-3.2-1B-Instruct-q4f16_0-MLC 6.36.51 PM.zip']) {
          final modelPath = '$modelsDir/$modelFileName';
          final modelFile = File(modelPath);
  ...
  static AIModel? _createModelFromFileName(String fileName, String filePath) {
    ...
    switch (fileName) {
      case 'Llama-3.2-1B-Instruct-q4f16_0-MLC 6.36.51 PM.zip':
        return AIModel(
      ...
      default:
        ...
        final baseId = fileName.replaceAll('.zip', '').replaceAll('.gguf', '').toLowerCase();
        print('ModelService: Creating generic model with ID: $baseId');
        return AIModel(
          id: baseId,
          name: fileName.replaceAll('.zip', '').replaceAll('.gguf', ''),
          description: 'Model file',
          version: 'Unknown',
          filePath: filePath,
          parameters: 0,
          format: fileName.contains('.zip') ? 'MLC' : 'GGUF',
          isAvailable: true,
        );
    }
  }
